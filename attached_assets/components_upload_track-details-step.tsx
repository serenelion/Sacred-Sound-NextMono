...
// Define metadata options (normally these would be in a separate constants file)
...
export function TrackDetailsStep({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        "Your content has been submitted for QA review. We'll notify you once it's approved."
      )
    } catch (error) {
            ...
            {files.map((track, index) => (
              <button
                ...
                className={`p-3 rounded-lg text-left transition-colors ${
                  ...
                  Object.keys(trackDetails[index]).every(key => 
                    trackDetails[index][key] !== '' && 
                    trackDetails[index][key]?.length !== 0
                  ) 
